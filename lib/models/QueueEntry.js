...
const ActionQueueEntry = require('./ActionQueueEntry');
        ...
        try {
            ...
            if (record.action) {
                return new ActionQueueEntry(
                    record.action, record.target, record.details);
            }
            // below, the entry is considered to hold a direct
            // representation of metadata from metadata storage
            let entry;
            if (record.type === 'del') {
