...
class IngestionProducer {
    ...
    /**
     * Validate remote access by checking if:
     ...
     * @return {undefined}
     */
    validateRemoteAccess(bucket, logger, done) {
        this._s3Client.getBucketVersioning({ Bucket: bucket }, (err, res) => {
            if (err) {
                ...
                if (err.code === 'NoSuchBucket') {
                    logger.error('remote bucket does not exist', logParams);
                } else if (err.code === 'InvalidAccessKeyId') {
                    logger.error('invalid access key provided', logParams);
                } else if (err.code === 'SignatureDoesNotMatch') {
                    // uncertain what errors this covers, but it does cover
                    // invalid secret key
                    logger.error('error uncertain, validate secret key',
                        logParams);
                } else if (err.code === 'UnknownEndpoint') {
                    logger.error('invalid endpoint', logParams);
                } else {
                    logger.error('error validating remote access', logParams);
                }
                return done(err);
            }
