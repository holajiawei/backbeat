...
class IngestionReader extends LogReader {
    ...
    _processReadRecords(params, batchState, done) {
        ...
        return async.waterfall([
            // Validate source information here because at this point, we have
            // setup internal clients in IngestionProducer, and can make a
            // simple `getBucketVersioning` to validate credentials work,
            // endpoint works, remote bucket exists, and that the bucket is
            // version enabled.
            // It is possible for the user to either delete the inaccurately
            // setup storage location (which we will cleanup), or the user
            // makes a change to their storage location (which we will edit)
            next => this._iProducer.validateRemoteAccess(this.bucket, logger,
                next),
            next => this._readInitState(logger, next), (initState, next) => {
