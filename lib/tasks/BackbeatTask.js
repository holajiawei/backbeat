...
const concurrencyConf =
      config.extensions.replication.queueProcessor.concurrency || {};

const locationTaskPools = {};

const DEFAULT_POOL_CONCURRENCY = 10;
...
class BackbeatTask {
    ...
    pool(locationType, actionFunc, done) {
        if (locationTaskPools[locationType] === undefined) {
            let concurrency = concurrencyConf[locationType];
            if (!concurrency) {
                concurrency = DEFAULT_POOL_CONCURRENCY;
            }
            locationTaskPools[locationType] = async.queue(
                (func, cb) => func(cb), concurrency);
        }
        locationTaskPools[locationType].push(actionFunc, done);
    }

    poolAndRetry(args, done) {
        const { locationType } = args;
        this.pool(locationType, cb => this.retry(args, cb), done);
    }
}
